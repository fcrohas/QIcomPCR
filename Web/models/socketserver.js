exports.SocketServer = Backbone.Model.extend({
	...
	onIcom2Node : function(data,client) {
	    ...
	    for (var i=0; i < frame.length; i++) {
	    	if (frame[i] !='') {
		      	if ((frame[i].substring(0,2) != 'WT') && (frame[i].substring(0,3) != 'BDS') && (client.allowScopeFrame == false)) {
		      	  client.volatile.send(frame[i]);
		      	}
		      	if ((client.allowScopeFrame == true) && ((frame[i].substring(0,2) == 'WT') || (frame[i].substring(0,3) == 'BDS'))) {
				  client.volatile.send(frame[i]);		      		
		      	}
	    	}
    	}
